...
import joblib
import pandas as pd
from lol_pipeline import feature_engineering  # usa mesma função usada no treino


def parse_args():
    p = argparse.ArgumentParser(description="Inferência com LogisticRegression")
    p.add_argument("--model_path", default="outputs/LogisticRegression_pipeline.joblib")
    p.add_argument("--data_path", required=True)
    p.add_argument("--output_path", default="predictions_logistic.csv")
    return p.parse_args()
